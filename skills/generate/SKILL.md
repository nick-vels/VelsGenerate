...
velsgenerate recommend image|video|audio [--refresh]   # подбор модели: последние версии
                                                       # популярных семейств с ценами и тирами
velsgenerate pricing [--refresh] [--category image|video|audio] [--search ТЕКСТ]  # цены в кредитах и $
...
1. **Узнай цену модели в кредитах (токенах) KIE.** Первый источник —
   `velsgenerate pricing --search МОДЕЛЬ --json`: живой прайс kie.ai (кэш 24ч),
   ...
   пользователю. Запасной источник — `docUrl` из `velsgenerate models --search`.
...
`velsgenerate recommend <категория> --json`: команда вернёт 3–4 самых популярных
