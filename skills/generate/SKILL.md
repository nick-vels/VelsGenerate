...
1. **Узнай цену модели в кредитах (токенах) KIE.** Открой `docUrl` из вывода
   `velsgenerate models --search ... --json` — на странице документации указана цена
   за генерацию — или страницу модели на kie.ai. Учитывай параметры запроса: цена часто
   зависит от длительности, разрешения, количества выходов и наличия аудио.
2. **Пересчитай в доллары** по актуальному курсу кредита KIE со страницы kie.ai/pricing.
   Курс не выдумывай и не бери из памяти — проверяй; если узнать не удалось, честно
   скажи, что оценка в долларах приблизительная или отсутствует.
3. **Сверься с балансом**: `velsgenerate credits`.
